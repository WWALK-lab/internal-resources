...
If using paper datasheets, make sure to take a photo of datasheets at the end of every day, to ensure they are backed up if they are lost/damaged.

If putting out passive sensors, make sure to create a datasheet that records the sensor IDs, locations, and date + time of deployment/retrieval.
...
-   private vs public: scientists generally publish our repositories as public as part of our commitment to open science. However, some data (and associated code) should NOT be publicly visible (e.g., interview data). Make sure to discuss this with your supervisor(s), collaborators, and consult your data sharing agreement when working with git and publicly archiving data.

## Post-data collection

### 1. Data back ups

Immediately upon data collection, you should be backing up your raw data files. If you are the person responsible for storing and archiving the data, your raw data should be stored on your computer, the lab computer, and a cloud system. If you are collecting your data over months or years, your back up system should be established as soon as you have collected any raw data and added to as data collection continues.

If you are collecting data in collaboration with community and/or government, make sure to consult your data sharing agreement and collaborators before archiving the data on the lab computer/cloud system.

**Personal Computers** Have your raw-data clearly labelled and in an easily-identifiable folder (see Project Repository section above). Treat the raw-data files as *read-only* and make copies of them before ever editing. **Never edit raw data directly**.

**Lab Computers** The lab computer in the WWALK lab can be accessed in person and remotely. Check details [here](/policies/04-lab-computer.qmd) on how to connect, so that you can back up your computer. The lab computer is also backed up by an external hard drive.
...
-   [ARDC Metadata Guide](https://zenodo.org/records/6459832)

-   [Metadata and describing data](https://data.research.cornell.edu/data-management/storing-and-managing/metadata/)
...
After finishing your project(s) in the lab, you are expected to deliver all data, metadata, and code that was used for your project to the appropriate place. If you are not the person responsible for data management, make sure that your data and metadata is sent to the person responsible in a clean and usable format. If you are responsible for data management, your data, metadata, and code should be archived in the lab and publicly (if appropriate).
...
If all of your code, data, and metadata is already archived publicly, this can be as easy as downloading a zip file and putting it on the lab computer. DETAILS.

If some of your data is not suitable to be publicly available, you may need to add it to your archived version on the lab computer and set permission restrictions on the folder on WHERE so that only yourself and Ally can access the folder.
